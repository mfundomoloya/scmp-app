  ...
  const MaintenanceAdmin = () => {
    ...
    const handleMaintenanceSubmit = async (e) => {
      ...
      try {
        ...
        console.log('MaintenanceAdmin: API URL:', `${import.meta.env.VITE_API_URL}/api/maintenance/room/${maintenanceForm.roomId}`);
        const response = await axios.put(
          `${import.meta.env.VITE_API_URL}/api/maintenance/room/${maintenanceForm.roomId}`,
          {
        ...
        setError(err.response?.data?.msg || 'Failed to update maintenance. Please try again.');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold mb-8 text-center text-white">Manage Maintenance Reports</h1>
        {error && (
            ...
            <table className="min-w-full bg-black bg-opacity-50 rounded-lg">
              <thead>
                <tr className="bg-gray-800 text-left">
                  ...
                  <th className="px-6 py-4 text-sm font-semibold text-gray-300">Reported By</th>
              ...
              <tbody className="divide-y divide-gray-800">
                {reports.map((report) => (
                  <tr key={report._id} className="hover:bg-gray-800">
                    <td className="px-6 py-4">{report.roomId?.name || 'Unknown'}</td>
                    <td className="px-6 py-4">{report.userId?.name || 'Unknown'}</td>
        ...
        {showMaintenanceModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gray-900 rounded-lg max-w-md w-full p-6">
              <h3 className="text-xl font-bold mb-4 text-white">Schedule Room Maintenance</h3>
