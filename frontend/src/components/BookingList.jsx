...
const BookingList = ({ refresh }) => {
  ...
  console.log('BookingList: Rendering for user:', user?.id, 'role:', user?.role);
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      console.log('BookingList: Fetching bookings with token:', token ? '[REDACTED]' : null);
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      if (modalAction.type === 'cancel') {
        ...
        console.log('BookingList: Updating booking status:', { id: showModal, status: modalAction.status });
        await axios.put(
          `${import.meta.env.VITE_API_URL}/api/bookings/${showModal}/status`,
          { status: modalAction.status },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        console.log('BookingList: Booking status updated:', { id: showModal, status: modalAction.status });
      }
      fetchBookings();
    } catch (err) {
      console.error(
                    ...
                    >
                      ...
                      {isAdmin && (
                        <>
                          <td className="px-6 py-4 whitespace-nowrap">
                             {booking.userId
                              ? `${booking.userId.firstName || ''} ${booking.userId.lastName || ''}`.trim() || 'Unknown'
                              : 'Unknown'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
